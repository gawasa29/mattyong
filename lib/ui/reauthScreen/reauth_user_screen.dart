...
class _ReAuthUserScreenState extends State<ReAuthUserScreen> {
  ...
  passwordButtonPressed() async {
    if (_passwordController.text.isEmpty) {
      showAlertDialog(
          context, 'Empty Password'.tr(), 'メールの更新にはパスワードが必要です。'.tr());
    } else {
      await showProgress(context, '削除中...'.tr(), false);
      try {
        ...
        if (result == null) {
          await hideProgress();
          showAlertDialog(
              context, 'Couldn\'t verify'.tr(), 'パスワードを再確認し、再度お試しください。.'.tr());
        } else {
          if (result.user != null) {
