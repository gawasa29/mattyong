...
class _ChatScreenState extends State<ChatScreen> {
                      ...
                      builder: (context, snapshot) {
                        if (snapshot.connectionState ==
                          ...
                          if (snapshot.hasData &&
                            ...
                            return Center(child: Text('まだメッセージはありません。'.tr()));
                          } else {
  ...
  // Widget buildSubTitle(UserInformation friend) {
  ...
  Widget buildMessage(MessageData messageData, List<UserInformation> members) {
  ...
  Widget remoteMessageView(MessageData messageData, UserInformation sender) {
    return Padding(
      padding: const EdgeInsets.only(top: 8.0),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.end,
  ...
  Future<bool> _checkChannelNullability(
    ...
    if (conversationModel != null) {
      ...
      UserInformation user = MyAppState.currentUser!;
          ...
          onPressed: () async {
            ...
            if (isSuccessful) {
              ...
              _showAlertDialog(
                  context,
                  'Block'.tr(),
                  '${homeConversationModel.members.first.lastName} ブロックしました。'
                      .tr());
            } else {
              _showAlertDialog(
                  context,
                  'Block'.tr(),
                  'Couldn'
                          '\'t block ${homeConversationModel.members.first.lastName}, 後でもう一度お試しください。.'
                      .tr());
            }
          },
        ),
          ...
          onPressed: () async {
            ...
            if (isSuccessful) {
              ...
              _showAlertDialog(
                  context,
                  'Report'.tr(),
                  '${homeConversationModel.members.first.lastName} が報告され、ブロックしました。.'
                      .tr());
            } else {
              _showAlertDialog(
                  context,
                  'Report'.tr(),
                  'Couldn'
                          '\'t report ${homeConversationModel.members.first.lastName}, 後でもう一度お試しください。.'
                      .tr());
            }
          },
        ),
