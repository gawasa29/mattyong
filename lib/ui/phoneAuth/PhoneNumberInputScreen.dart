...
class _PhoneNumberInputScreenState extends State<PhoneNumberInputScreen> {
          ...
          child: Form(
            ...
            child: Column(
              children: <Widget>[
                ...
                /// ユーザーのプロフィール写真は、確認するまで表示されます。
                /// 電話番号でサインアップした場合の コード
                Padding(
                ...
                /// code in case of sign up with phone number
                /// user first name text field , これは、電話番号を確認するまで表示されます。
                ///電話番号でサインアップした場合の  コード
                Visibility(
                  visible: !_codeSent && !widget.login,
                ...
                /// コード検証フィールド、これはサインアップ時に
                /// 電話番号を入力すると、コードが送信されます
                Visibility(
                  visible: _codeSent,
                  child: Padding(
                    padding:
                ...
                /// 画面のメインアクションボタンで、以下の場合は非表示になります。
                /// firebaseからコードを受信した場合は隠されます。
                /// アクションとタイトルは状態に応じて変化します。
                /// * Sign up with email and password: Eメールとパスワードを
                /// firebase
                /// * 電話番号でサインアップ: 電話番号を /// firebase に送信します。
                /// firebaseに電話番号を送信し、コードの確認を待ちます。
                Visibility(
                  visible: !_codeSent,
                  child: Padding(
                    padding: const EdgeInsets.only(
                ...
                /// 電話番号でのサインアップとメールでのサインアップの状態を切り替える
                InkWell(
                  onTap: () {
  ...
  /// コードを firebase に送信して検証を受けた後、ユーザーの
  /// firebase データベースからオブジェクトを取得します。
  /// @param code code フィールドからの入力コードを取得します。
  /// 電話でのログインから新しいユーザを作成します
  ...
  /// is lost
  /// アンドロイドでは、イメージピッカーリブによって使用されていますが、時々、画像が
  ///が失われることがあります。
  ...
  /// プロフィールを選択しようとしたときに表示されるメニューオプションのセット
  /// ギャラリーから画像をダウンロードするか、新しい画像を撮影する
  _onCameraClick() {
  ...
  /// 電話番号を使用して新しいユーザーを作成するためにfirebaseにリクエストを送信します。
  ///区の後に[ContainerScreen]にナビゲートする
    ...
    await FireStoreUtils.firebaseSubmitPhoneNumber(
      ...
      (auth.PhoneAuthCredential credential) async {
        if (mounted) {
          ...
          UserInformation? user = await FireStoreUtils.getCurrentUser(
